...
It changed `subtract` from `return a - b` to `return a + b`.
...
This creates a new commit that reverses the changes from commit 3, restoring `subtract` to `return a - b`. The bad commit stays in history so you can see what happened. The revert commit explains the fix.
